...
import melonystudios.behaviorapi.SoundSourceCodec;
...
public class PlaySoundBehavior extends ItemBehavior {
    ...
    public PlaySoundBehavior() {
        this(new PlaySound(BehaviorAPI.PLAY_SOUND_DEFAULT, SoundSourceCodec.MASTER, Vec3.ZERO));
    }
    ...
    public void runBehavior(ItemStack stack, Level world, LivingEntity livEntity) {
        ...
        if (livEntity instanceof ServerPlayer player) {
            player.connection.send(new ClientboundSoundPacket(sound, this.playSound.source().decodec(), truePos.x, truePos.y, truePos.z, trueVolume, this.playSound.pitch(), seed));
        } else {
            world.playSound(null, truePos.x, truePos.y, truePos.z, sound.value(), this.playSound.source().decodec(), trueVolume, this.playSound.pitch());
        }
    }
